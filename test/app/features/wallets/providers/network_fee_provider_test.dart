...
void main() {
  group('NetworkFeeProvider', () {
    group('canUserCoverFee function', () {
      ...
      group('with sendAmount parameter', () {
        test('returns false when user cannot cover fee + send amount for native tokens', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'BTC',
            balance: '10000', // 0.0001 BTC in satoshis
            decimals: 8,
          ...
          expect(
            canUserCoverFee(
              selectedFee: selectedFee,
              networkNativeToken: networkNativeToken,
              sendAmount: 0.0001, // Trying to send 0.0001 BTC
              isSendingNativeToken: true,
            ),
            isFalse,
          );
        });

        test('returns true when user can cover fee + send amount for native tokens', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'BTC',
            balance: '50000000', // 0.5 BTC in satoshis
            decimals: 8,
          ...
          expect(
            canUserCoverFee(
              selectedFee: selectedFee,
              networkNativeToken: networkNativeToken,
              sendAmount: 0.1, // Trying to send 0.1 BTC
              isSendingNativeToken: true,
            ),
            isTrue,
          );
        ...
        test('ignores send amount when not sending native tokens', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'ETH',
            balance: '1000000000000000', // 0.001 ETH in wei
            decimals: 18,
            kind: 'Native',
          );
          const selectedFee = NetworkFeeOption(
            amount: 0.0005, // 0.0005 ETH
            priceUSD: 0.3,
            symbol: 'ETH',
            type: NetworkFeeType.standard,
          );
          ...
          expect(
            canUserCoverFee(
              selectedFee: selectedFee,
              networkNativeToken: networkNativeToken,
              sendAmount: 10, // Large send amount but we're not sending native tokens
              // isSendingNativeToken defaults to false
            ),
            isTrue,
          );
        });

        test('handles edge case where balance exactly equals fee + send amount', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'BTC',
            balance: '11000', // 0.00011 BTC in satoshis
            decimals: 8,
            kind: 'Native',
          );
          const selectedFee = NetworkFeeOption(
          ...
          expect(
            canUserCoverFee(
              selectedFee: selectedFee,
              networkNativeToken: networkNativeToken,
              sendAmount: 0.00001, // 0.00001 BTC, total = 0.00011 BTC
              isSendingNativeToken: true,
            ),
            isTrue,
          );
        ...
        test('handles case where selectedFee is null but user wants to send native tokens', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'ETH',
            balance: '500000000000000000', // 0.5 ETH in wei
            decimals: 18,
          ...
          expect(
            canUserCoverFee(
              selectedFee: null,
              networkNativeToken: networkNativeToken,
              sendAmount: 0.1, // Trying to send 0.1 ETH
              isSendingNativeToken: true,
            ),
            isTrue,
          );
        ...
        test('handles case where selectedFee is null and user cannot send native tokens', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'ETH',
            balance: '50000000000000000', // 0.05 ETH in wei
            decimals: 18,
            kind: 'Native',
          );
          ...
          expect(
            canUserCoverFee(
              selectedFee: null,
              networkNativeToken: networkNativeToken,
              sendAmount: 0.1, // Trying to send 0.1 ETH but only have 0.05
              isSendingNativeToken: true,
            ),
            isFalse,
          );
        ...
        test('defaults work correctly - backward compatibility', () {
          const networkNativeToken = ion.WalletAsset.native(
            symbol: 'BTC',
            balance: '100000000', // 1 BTC in satoshis
            decimals: 8,
          ...
          // Call without new parameters - should work like before
          expect(
            canUserCoverFee(
              selectedFee: selectedFee,
              networkNativeToken: networkNativeToken,
